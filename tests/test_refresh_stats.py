...
class HumanTests(unittest.TestCase):
    def test_human_formats_each_magnitude(self) -> None:
        cases = [
            (0, "0"),
            (999, "999"),
            (1_000, "1.0k"),
...
class ReplaceAfterLabelTests(unittest.TestCase):
    def test_plain_replacement(self) -> None:
    ...
    def test_missing_label_is_noop(self) -> None:
...
class ReplaceStarsBadgeTests(unittest.TestCase):
    ...
    def test_missing_badge_is_noop(self) -> None:
        out, ok = rs.replace_stars_badge("no badge", 250)
        self.assertFalse(ok)
        self.assertEqual(out, "no badge")


class CountSectionEntriesTests(unittest.TestCase):
    def test_stops_at_divider(self) -> None:
        md = (
    ...
    def test_runs_to_end_of_string_without_divider(self) -> None:
        md = "### Section\n| [A](u) | x |\n| [B](u) | y |\n"
        self.assertEqual(rs.count_section_entries(md, r"^### Section", "| ["), 2)


class CountDistinctBoldedTests(unittest.TestCase):
    def test_filters_numeric_and_dedupes(self) -> None:
        md = (
            "### Hackathon Submissions\n"
    ...
    def test_missing_header_returns_zero(self) -> None:
        self.assertEqual(rs.count_distinct_bolded("nope", r"^### Missing"), 0)


class CountBadgeImagesTests(unittest.TestCase):
    def test_counts_only_within_section(self) -> None:
        md = (
            "### Certifications\n"
            "![AWS](https://x/a.svg)\n"
            "some prose\n"
    ...
    def test_missing_header_returns_zero(self) -> None:
        self.assertEqual(rs.count_badge_images("no header", r"^### Missing"), 0)


class WriteShieldEndpointTests(unittest.TestCase):
    def test_writes_valid_payload_and_creates_dirs(self) -> None:
...
class RecentlyShippedTests(unittest.TestCase):
    ...
    def test_render_npm_registry_label(self) -> None:
        releases = [
            {
                "date": "2026-06-01",
                "name": "pkg",
        ...
        out = rs.render_recently_shipped(releases, [])
